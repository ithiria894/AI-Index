...
## Step 2 — Locate the origin symbol

Before opening any file, find the exact function/class you need.

**If you know the symbol name** — use LSP for precision:
```
findReferences: src/<domain>/<entry>.py scope=<SymbolName>
```

**If you only have a keyword or error message** — search for the entry point:
```
LSP workspaceSymbol: <keyword>         ← finds matching symbol definitions
grep (fallback): <error_message_text>  ← if LSP finds nothing
```

**If LSP is unavailable**, fall back to grep/glob:
```
